                ...
                {filteredItems.length === 0 ? (
                  ...
                  return (
                    <button
                      key={item.id}
                      onClick={() => setSelectedId(item.id)}
                      className={`flex-shrink-0 w-60 text-left p-3 rounded-xl border transition-all ${
                        isSelected
                          ? 'bg-blue-50 border-blue-400 shadow-sm'
                          : `bg-white hover:bg-gray-50 ${accent} hover:border-gray-400`
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <p className="text-xs font-bold text-gray-900 truncate">{item.customerName}</p>
                        </div>
                        {item.priority === 'high' && (
                          <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 bg-red-100 text-red-700 rounded flex-shrink-0">
                            High
                          </span>
                        )}
                      </div>
                      {item.startTime && (
                        <p className="text-[11px] font-semibold text-gray-700 mb-0.5">{timeStr(item.startTime)}</p>
                      )}
                      {item.subtitle && (
                        <p className="text-[11px] text-gray-500 truncate">{item.subtitle}</p>
                      )}
                      {item.assignedTo && (
                        <div className="flex items-center gap-1 mt-1.5 text-[11px] text-gray-500 truncate">
