/*
# Create company_profile table and update NFPA 72 template

## Summary
1. Creates a `company_profile` table to store the servicing company's own information
   (name, address, phone, license) used to pre-fill inspection forms.
2. Seeds one demo row for the prototype.
3. Updates the NFPA 72 template to:
   - Move "Central Station Notified After Testing?" from Page 1 to Page 4
   - Add a "Photo Documentation" section to Pages 2 and 3
   - Add a "Post-Inspection Notifications" section before Signatures on Page 4
...
## Security
- RLS enabled on `company_profile` with anon + authenticated CRUD
*/
...
CREATE POLICY "anon_select_company_profile" ON company_profile FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "anon_insert_company_profile" ON company_profile;
CREATE POLICY "anon_insert_company_profile" ON company_profile FOR INSERT
  TO anon, authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "anon_update_company_profile" ON company_profile;
CREATE POLICY "anon_update_company_profile" ON company_profile FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "anon_delete_company_profile" ON company_profile;
CREATE POLICY "anon_delete_company_profile" ON company_profile FOR DELETE
  TO anon, authenticated USING (true);

-- 2. Seed demo company profile
INSERT INTO company_profile (name, address, city, state, zip, phone, license_number)
VALUES ('WorkHorse Security & Low Voltage', '1234 Commerce Blvd, Suite 200', 'Dallas', 'TX', '75201', '(214) 555-0199', 'ACR-2024-78541')
ON CONFLICT DO NOTHING;

-- 3. Update the NFPA 72 template pages JSON
...
SET pages = '[
  {
    ...
    "sections": [
      ...
      {
        "title": "Monitoring / Central Station",
        "fields": [
          {"id": "mon_central_station", "label": "Central Station Name", "type": "text"},
          {"id": "mon_account_number", "label": "Monitoring Account #", "type": "text", "prefill": "alarm_account"},
          ...
          {"id": "mon_on_test", "label": "System Placed on Test?", "type": "yes_no"},
          {"id": "mon_test_duration", "label": "Test Duration (hours)", "type": "number", "conditional": {"field": "mon_on_test", "value": true}}
        ]
      }
    ]
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "description": "Attach photos documenting the visual inspection findings, deficiencies, or notable conditions.",
        "fields": [
          {"id": "vi_photos", "label": "Visual Inspection Photos", "type": "photo_gallery"}
        ]
      }
    ]
  },
  {
    ...
    "sections": [
      ...
      {
        "title": "Photo Documentation",
        "description": "Attach photos documenting testing results, issues, or conditions found during functional testing.",
        "fields": [
          {"id": "ft_photos", "label": "Functional Testing Photos", "type": "photo_gallery"}
        ]
      }
    ]
  },
  {
    ...
    "sections": [
      ...
      {
        "title": "Photo Documentation",
        "description": "Attach any additional photos documenting deficiencies or the completed inspection.",
        "fields": [
          {"id": "sum_photos", "label": "Summary / Deficiency Photos", "type": "photo_gallery"}
        ]
      },
      {
        "title": "Post-Inspection Notifications",
        "description": "Complete this section after all testing is finished.",
        "fields": [
          {"id": "mon_restored", "label": "Central Station Notified After Testing?", "type": "yes_no", "required": true},
          {"id": "mon_restored_time", "label": "Time Notified", "type": "time"},
          {"id": "mon_restored_person", "label": "Person Notified", "type": "text"}
        ]
      },
      {
